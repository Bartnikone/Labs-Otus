...
```
Итак, Spine_1/2 являются участниками зоны Level-1-2 и будут служить связующим звеном для роутеров Level-1 и Level-2.
...
AFI (Authority and Format Identifier) — 1 байт (2 шестнадцатеричные цифры).

Area ID — идентификатор области (обычно 2-4 байта).

System ID — уникальный идентификатор узла (6 байт).

SEL (NSAP Selector) — 1 байт (всегда 00 для маршрутизаторов).
